...
function Digit({ value, onInc, onDec }: { value: number; onInc: () => void; onDec: () => void }) {
  const y = useRef(new Animated.Value(0)).current;

  const bump = (dir: 1 | -1) => {
    Animated.sequence([
      Animated.timing(y, { toValue: dir * -8, duration: 120, useNativeDriver: true }),
  ...
  return (
    <View style={styles.digitWrap}>
      <TouchableOpacity onPress={() => { onInc(); bump(1); }} style={styles.arrow}>
        <Text style={styles.arrowTxt}>▲</Text>
      </TouchableOpacity>
      <Animated.View style={[styles.digit, { transform: [{ translateY: y }] }]}>
        <Text style={styles.digitTxt}>{value}</Text>
      </Animated.View>
      <TouchableOpacity onPress={() => { onDec(); bump(-1); }} style={styles.arrow}>
        <Text style={styles.arrowTxt}>▼</Text>
      </TouchableOpacity>
    </View>
  );
  ...
  return (
    <SafeAreaView style={[styles.container, { backgroundColor: BRAND.blue }]}>
      ...
      <View style={styles.center}>
        ...
        <Text style={styles.valueLabel}>{d2}{d1}{d0}</Text>
