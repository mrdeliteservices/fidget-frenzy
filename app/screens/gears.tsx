...
import { View, Text, StyleSheet, Animated, Image, TouchableOpacity, SafeAreaView } from "react-native";
...
export default function Gears() {
  ...
  const toggle = async () => {
    ...
    setRunning((r) => {
      if (r) { spinA.stopAnimation(); spinB.stopAnimation(); }
      else { loop(spinA, 4000).start(); loop(spinB, 6000).start(); }
      return !r;
    });
  };
    ...
    <SafeAreaView style={[styles.container, { backgroundColor: BRAND.blue }]}>
      ...
      <View style={styles.center}>
        ...
        <TouchableOpacity style={styles.button} onPress={toggle} activeOpacity={0.85}>
          <Text style={styles.buttonTxt}>{running ? "Pause" : "Resume"}</Text>
        </TouchableOpacity>
...
const styles = StyleSheet.create({
  ...
  button: {
    marginTop: 24, paddingHorizontal: 24, paddingVertical: 12, borderRadius: 22,
    backgroundColor: BRAND.purple, shadowColor: BRAND.gold, shadowOpacity: 0.6, shadowRadius: 12, shadowOffset: { width: 0, height: 0 }
  },
